...
const AddAudience = () => {
    ...
    const objSchema = {}
    rowUser.forEach( row => {
        objSchema[`fullName${row.id}`] = yup.string().required()
        objSchema[`email${row.id}`] = yup.string().email().required()
        objSchema[`number${row.id}`] = yup.number().positive().integer().required()
                ...
                <form onSubmit = {handleSubmit(saveData)} noValidate autoComplete='off'>
                    ...
                    {rowUser.map((row, index)=>(
                     <div className={classes.inputsSection}>
                        ...
                        {index===0 ? '' : <IconDelete handlerDeleteRow={handlerDeleteRow} delId={row.id} />}
                    </div>
                    ))}
